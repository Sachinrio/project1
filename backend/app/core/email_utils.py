...
async def send_sendgrid_email(to_email: str, subject: str, html_part: str, attachments: list = None):
    """
    Core helper to send emails via SendGrid API
    """
    print(f"DEBUG: Attempting to send SendGrid email to {to_email}...")
    
    if not SENDGRID_API_KEY:
        print("ERROR: SENDGRID_API_KEY is missing from environment.")
        return False
    ...
    content = Content("text/html", html_part)
    ...
    if attachments:
        for att in attachments:
            try:
                # att format: {"ContentType": "...", "Filename": "...", "Base64Content": "..."}
                encoded_file = att.get("Base64Content")
                file_name = att.get("Filename")
                file_type = att.get("ContentType")
                ...
                print(f"ERROR: Failed to add SendGrid attachment {att.get('Filename')}: {e}")

    try:
        # Run in thread since sendgrid client is synchronous
        response = await asyncio.to_thread(sg.send, mail)
        if response.status_code in [200, 201, 202]:
            print(f"SUCCESS: SendGrid sent email to {to_email}")
            return True
        else:
            print(f"ERROR: SendGrid failed with status {response.status_code}")
            print(f"ERROR BODY: {response.body}")
            print(f"ERROR HEADERS: {response.headers}")
            return False
    except Exception as e:
        print(f"EXCEPTION sending via SendGrid: {type(e).__name__}: {e}")
        if hasattr(e, 'body'):
...
async def send_mailjet_email(to_email: str, subject: str, html_part: str, attachments: list = None):
    return await send_sendgrid_email(to_email, subject, html_part, attachments)

async def send_smtp_email(to_email: str, subject: str, html_part: str, attachments: list = None):
    return await send_sendgrid_email(to_email, subject, html_part, attachments)
...
async def send_reset_email(email: EmailStr, otp: str):
    ...
    """
    return await send_smtp_email(email, "Password Reset Request - Infinite BZ", html_content)
...
async def send_verification_email(email: EmailStr, otp: str):
    ...
    """
    return await send_smtp_email(email, "Verify your email - Infinite BZ", html_content)
...
async def send_ticket_email(email: EmailStr, name: str, event_title: str, event_id: int, ticket_id: str = "", ticket_path: str = None):
    ...
    """
    return await send_smtp_email(email, f"Successfully Registered: {event_title}", body, attachments=attachments)
...
async def send_organizer_notification_email(email: EmailStr, organizer_name: str, attendee_name: str, attendee_email: str, event_title: str, event_date, ticket_path: str):
    try:
        ...
        """
        return await send_smtp_email(MAIL_FROM, f"New Registration: {event_title} - {attendee_name}", body, attachments=[attachment])
    except Exception as e:
...
async def send_event_ticket_email(email: EmailStr, event_data: dict, confirmation_id: str = None, user_name: str = None):
    try:
        ...
        """
        return await send_smtp_email(email, f"Your Event Ticket for {event_data.get('title', 'Event')}", body, attachments=[attachment])
    except Exception as e:
