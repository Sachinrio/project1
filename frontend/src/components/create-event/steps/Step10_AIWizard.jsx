...
import { Sparkles, Calendar, Clock, MapPin, DollarSign, ChevronRight, LayoutTemplate } from 'lucide-react';
                    ...
                    <div className="flex-1 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="relative group">
                                <Calendar className="absolute right-3 top-1/2 -translate-y-1/2 text-indigo-400 pointer-events-none z-10 opacity-70 group-hover:opacity-100 transition-opacity" size={18} />
                                <input
                                    type="date"
                                    min={new Date().toISOString().split('T')[0]}
                                    value={formData.startDate}
                                    onChange={(e) => updateFormData({ startDate: e.target.value })}
                                    className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 pr-10 text-white focus:ring-2 focus:ring-indigo-500 outline-none custom-picker-input"
                                />
                            </div>
                            <div className="relative group">
                                <Clock className="absolute right-3 top-1/2 -translate-y-1/2 text-indigo-400 pointer-events-none z-10 opacity-70 group-hover:opacity-100 transition-opacity" size={18} />
                                <input
                                    type="time"
