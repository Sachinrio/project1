...
const App: React.FC = () => {
  ...
  return (
    <>
      ...
      <div ref={rootRef} className={`flex flex-col min-h-screen transition-opacity duration-1000 ${showIntro ? 'opacity-0 overflow-hidden h-screen' : 'opacity-100'}`}>
        <Header onExport={exportPageAsPng} />
              ...
              <aside className="lg:w-96 space-y-12 reveal" style={{ animationDelay: '0.6s' }}>
                <div className="sticky top-28 space-y-12">
                  ...
                  <div className="bg-slate-900 p-10 rounded-[2.5rem] text-white shadow-2xl shadow-indigo-200/20 group relative overflow-hidden">
                    ...
                    <button className="bg-white text-slate-950 px-8 py-4 rounded-2xl font-black text-sm hover:bg-indigo-500 hover:text-white transition-all btn-bounce w-full">
                      Create Event
                    </button>
                  </div>
                </div>
...
const PulseItem: React.FC<{ label: string; value: string; trend: string; isSpecial?: boolean }> = ({ label, value, trend, isSpecial }) => (
